...
async function loadAdminCats(){const {data,error}=await db.from("cats").select("*").eq("moderation_status",adminFilter).order("created_at",{ascending:false});$("#adminCats").innerHTML=error?'<div class="empty">Nie udało się pobrać zgłoszeń.</div>':(data||[]).length?(data||[]).map(c=>dashItem(c,true)).join(""):'<div class="empty">Brak ogłoszeń w tej kategorii.</div>'}
